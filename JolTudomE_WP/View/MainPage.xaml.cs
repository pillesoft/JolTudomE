  ...
  public sealed partial class MainPage : Page {
    ...
    private async void NavigationHelper_LoadState(object sender, LoadStateEventArgs e) {
      ...
      if (!wm.IsAuthenticated()) {
        if (await wm.Login("Ivanka", "12345678")) {
          ((MainViewModel)this.DataContext).User = wm.LoggedInUser;
        }
      }
      ((MainViewModel)this.DataContext).StatisticList = await wm.GetStatistics(wm.LoggedInUser.PersonID);
      ((MainViewModel)this.DataContext).CourseList = await wm.GetCourses();
